...
# 分桶:[0,8) step=0.5(16 桶)+ 一个 >8 桶;y 轴为占比(%)
hedges=[round(0.5*i,1) for i in range(17)]+[1e9]
...
ax.set_xticks([0,4,8,12,16]); ax.set_xticklabels(["0","2","4","6",">8"])
ax.set_xlabel("|predicted jump − true jump| (kcal/mol, d=1; bin=0.5 in [0,8], last bar = >8)")
ax.set_ylabel("percentage of d=1 pairs (%)")
ax.set_title(f"d=1 jump-prediction error (n={len(d1e)}; median={d1e.jerr.median():.2f}, p90={d1e.jerr.quantile(.9):.2f}, >8={100*(d1e.jerr>8).mean():.1f}%)")
...
ax.set_xticks([0,4,8,12,16]); ax.set_xticklabels(["0","2","4","6",">8"])
ax.set_xlabel("|predicted jump − true jump| bin (kcal/mol, d=1; 0.5 in [0,8], last bar = >8)")
