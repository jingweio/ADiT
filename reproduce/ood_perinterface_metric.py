...
spearman 再取均值。
用法: python reproduce/ood_perinterface_metric.py <label> <pkl1> [pkl2 ...]
"""
import sys, pickle
import numpy as np
...
MIN_N = 2  # 组内样本数阈值(<2 无法算 spearman)
...
def main():
    ...
    print(f"### {label}")
    print(f"  n_samples={n} | n_complexes={len(groups)} | per-iface used={used} "
          f"(skipped const/<{MIN_N}: {skipped_const}+{len(groups)-used-skipped_const})")
    print(f"  OVERALL: pearson={pe:.4f} spearman={sp:.4f} rmse={rmse:.4f} mae={mae:.4f}")
    print(f"  PER-INTERFACE Spearman: mean={mean_per_iface:.4f} median={median_per_iface:.4f}")
